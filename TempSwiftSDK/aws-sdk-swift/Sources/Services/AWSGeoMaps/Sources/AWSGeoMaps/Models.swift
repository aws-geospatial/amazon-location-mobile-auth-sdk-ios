...
public struct GetGlyphsInput: Swift.Sendable {
    /// Name of the FontStack to retrieve. For example: Amazon Ember Bold,Noto Sans Bold. A full list of supported font stacks is below:
    ///
    /// * Noto Sans Bold
    ///
    /// * Noto Sans Italic
    ...
    /// A Unicode range of characters to download glyphs for, must be aligned to multiples of 256. example: 0-255
    /// This member is required.
...
public struct GetStaticMapInput: Swift.Sendable {
    /// Takes in 2+ pair of coordinates [Lon, Lat] with each coordinate separated by comma. The API will generate an image to encompass all of the provided coordinates. Example: 27.170451,78.039098,27.176178,78.045536
    public var boundedPositions: Swift.String?
    /// Takes in 2 pairs of coordinates [Lon, Lat] denoting southwesterly and northeasterly edges of the image. The underlying area becomes the view of the image. Example: -123.12392699629073,49.28523757791812,-123.11427463431049,49.270344443142534
    public var boundingBox: Swift.String?
    /// Takes in a pair of coordinates [Lon, Lat] which becomes the center point of the image. This parameter requires that either zoom or radius is set. Example:49.295,-123.108
    public var center: Swift.String?
    /// Takes in a string to draw geometries on the image. The input is in format geometry_type:geometry;property_1=value_1,property_2=value_2|geometry_type:geometry;property_1=value_1… Example: point:-13.2223,53.0123;label=A|point:-13.2233,53.0153;label=B
    ...
    /// This member is required.
    public var fileName: Swift.String?
    /// Takes in a string to draw geometries on the image. The input is a valid GeoJSON collection object. Example: {"type":"FeatureCollection","features":[{"type":"Feature","geometry":{"type":"MultiPoint","coordinates":[[-990.076345,51.504107],[-0.074451,51.506892]]},"properties":{"color":"#00DD00"}}]}
    ...
    /// Applies additional space around overlay feature to prevent them from being cut or obscured (Specified in pixels). Example: 100
    public var padding: Swift.Int?
    /// Used with center parameter, it specifies the zoom of the image where you can control it on a granular level. Takes in any value >= 1. Example: 1500
    public var radius: Swift.Int?
    /// Displays a scale on the bottom right of the map image with the unit specified in the input. Example: KilometerMiles, Miles, Kilometer, MilesKilometers
...
public struct GetStaticMapOutput: Swift.Sendable {
    /// The blob represents a map image in format of jpeg or png for the GetStaticMap API.
    public var blob: Foundation.Data?
    /// Header that instructs caching configuration for the client.
    public var cacheControl: Swift.String?
    /// Header that represents the format of the response. The response returns the following as the HTTP body.
...
public struct GetStyleDescriptorInput: Swift.Sendable {
    ...
    /// Optional: The API key to be used for authorization. Optional, either an API key or valid SigV4 signature must be provided when making a request.
    public var key: Swift.String?
    /// Specifies the political view following ISO 3166-2 or ISO 3166-3 country code format. The following political views are currently supported:
    ///
...
extension GetStyleDescriptorInput: Swift.CustomDebugStringConvertible {
    public var debugDescription: Swift.String {
        "GetStyleDescriptorInput(colorScheme: \(Swift.String(describing: colorScheme)), politicalView: \(Swift.String(describing: politicalView)), style: \(Swift.String(describing: style)), variant: \(Swift.String(describing: variant)), key: \"CONTENT_REDACTED\")"}
}
...
public struct GetTileInput: Swift.Sendable {
    ...
    /// The Z axis value for the map tile.
    /// This member is required.
...
extension GetStyleDescriptorInput {
    ...
    static func queryItemProvider(_ value: GetStyleDescriptorInput) throws -> [Smithy.URIQueryItem] {
        var items = [Smithy.URIQueryItem]()
        if let variant = value.variant {
            let variantQueryItem = Smithy.URIQueryItem(name: "variant".urlPercentEncoding(), value: Swift.String(variant.rawValue).urlPercentEncoding())
            items.append(variantQueryItem)
        }
